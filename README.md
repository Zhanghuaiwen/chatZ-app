...
- **深色主题**：护眼深色 UI，类 ChatGPT 设计风格
- **快捷操作**：欢迎页预设快捷提问按钮
- **移动端适配**：响应式布局，移动端可折叠侧边栏
- **连接测试**：设置面板内置 Ollama 连接测试按钮，快速诊断问题
- **本地持久化**：对话记录和设置自动保存至浏览器 localStorage
...
5. **发送消息**：输入文本后按 Enter 发送，Shift+Enter 换行
6. **停止生成**：AI 回复时可点击红色方块按钮停止流式输出
7. **管理对话**：侧边栏显示所有对话，可点击切换、悬停显示删除按钮
